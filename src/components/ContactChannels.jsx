 ...
 * About/Experience"). Same markup, state and i18n keys as before — this is
 * the channels/CV/tags/personal-cards/closeout block, self-contained
 * (including its own lightbox state) so both Contact.jsx (Home, unchanged)
 * and components/ContactView.jsx (the standalone `#contact` destination)
 * can render it independently.
 *
 * The lightbox (`.card-lightbox`) is `position: fixed` (styles.css), so
 * rendering it as a sibling of `.contact-main` here is safe wherever this
 * component is placed — CSS Grid excludes out-of-flow elements from track
 * layout, so it never becomes an unwanted grid cell in Contact.jsx's
 * `.contact-grid`.
 *
 * `compact` (Phase 3, item 8): ContactView passes `compact` to keep the
 * standalone `#contact` destination "la página más simple de las cuatro" —
 * it hides the personal business-card images/lightbox and the editorial
 * "closeout" aside (a Selected-Work-style note), neither of which is
 * "cómo contactar / dónde / modalidad / disponibilidad / CV". Defaults to
 * `false` so nothing changes for any other caller (there is none currently
 * — Contact.jsx, Home's old full section, is unreferenced since Phase 2 —
 * but the default keeps this component's behavior unchanged either way).
 */
...
const channelMeta = [
  ...
  {
    id: 'github',
    href: 'https://github.com/AgusDelgado98',
    accent: 'violet',
    handle: 'AgusDelgado98',
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
  ...
  return (
    <>
      <div className="contact-main card card--spotlight fade-in fade-in-delay-1">
        <h2 className="contact-main-title">{t('contact.pointsTitle')}</h2>
